...
class DQN:
    def __init__(self, state_dim=None, action_dim=None, discount=0.9):
        ...
        self.Q = DuelingQnet(state_dim, action_dim).to(device)
        self.target_Q = DuelingQnet(state_dim, action_dim).to(device)
...
def task_train(env):
    ...
    with open(f"./DuelingDQN_data/delta_{env.delta}_duelingdqn.txt", "w") as f:
