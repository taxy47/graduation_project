...
def task_train(env):
    ...
    for episode in range(num_episodes):
        ...
        episode_loss_list.append(episode_loss / max_step)      
        print("\n")
    ...
    with open(f"./DQN_data/delta_{env.delta}_dqn.txt", "w") as f:
        for i in range(num_episodes):
            f.write(f"{episode_reward_list[i]}\n")

    plt.figure(figsize=(5, 5))
    plt.plot(x, episode_reward_list, label='test for training')
    # plt.plot(x, episode_loss_list, label='test for loss') # 画图的 y 轴
    # 保存图像，训练非阻塞
