...
class DQN:
    ...
    def compute_loss(self, s_batch, a_batch, r_batch, d_batch, next_s_batch):
        ...
        # next_qvals = self.target_Q(next_s_batch).max(dim=1)[0] # dqn

                # 用主网络选择动作
        next_actions = self.Q(next_s_batch).argmax(dim=1, keepdim=True)  # shape: [batch_size, 1]

        # 用目标网络评估这些动作的价值
        next_qvals = self.target_Q(next_s_batch).gather(1, next_actions).squeeze(1) # ddqn
...
def task_train(env):
    ...
    with open(f"./DDQN_data/delta_{env.delta}_ddqn.txt", "w") as f:
